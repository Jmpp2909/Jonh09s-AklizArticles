# Akliz Blog Articles Backup/Sync

Howdy, this is the repository I'll use to sync/backup all my articles written for Akliz.  
I use [StackEdit.io](https://stackedit.io) for drafting if you’re interested.

---

## License

All contents of this repository are licensed under **All Rights Reserved (ARR)**.  

This means:  
- You may **not** copy, redistribute, modify, or use any part of this repository without my explicit written permission.  
- All rights remain with the author.  
- Usage outside of personal viewing is prohibited unless granted in writing.  

© [Your Name], 2025. All rights reserved.
